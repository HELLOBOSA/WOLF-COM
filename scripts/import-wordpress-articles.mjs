...
<body>
<nav id="nav"></nav>
<header class="art-header"><a href="../" class="back-link">← Journal</a><div class="art-meta"><span>${item.category}</span><span class="dot"></span><span class="dim">${displayDate(post.date)}</span><span class="dot"></span><span class="dim">Wolfblanc Architects</span></div><h1>${item.title}</h1><p class="art-lead">${item.excerpt}</p><div class="art-byline"><div class="byline-avatar"><img src="../../images/wolfblanc-team.webp" alt="Wolfblanc Architects" width="34" height="34"></div><p class="byline-text"><strong>Wolfblanc Architects</strong><br>Spain · Sweden · Greece</p></div></header>
<div class="hero-img"><div class="hero-img-wrap"><img src="../../${item.localImage}" alt="${attr(item.title)}" loading="eager" fetchpriority="high" decoding="async"></div><p class="img-caption">Wolfblanc Journal · ${item.category}</p></div>
